...
public sealed class SpawnRandomPlanet : Job<bool>
{
    ...
    public SpawnRandomPlanet(
        ...
        CancellationToken cancellation = default) : base(maxTime: maxTime, cancellation)
    {
    ...
    {
        _sawmill.Debug("salvage", $"Spawning salvage mission with seed {_planetParams.Seed}");
        var mapUid = _map.CreateMap(out var mapId, runMapInit: false);
        MetaDataComponent? metadata = null;
        var grid = _entManager.EnsureComponent<MapGridComponent>(mapUid);
        var random = new Random(_planetParams.Seed);
        var destComp = _entManager.AddComponent<FTLDestinationComponent>(mapUid);
        destComp.BeaconsOnly = true;
        destComp.Enabled = true;
        _metaData.SetEntityName(
            mapUid,
            _entManager.System<SharedSalvageSystem>().GetFTLName(_prototypeManager.Index(SalvageSystem.PlanetNames), _planetParams.Seed));
        ...
        var mission = _entManager.System<SharedSalvageSystem>()
            .GetMission(difficultyProto, _planetParams.Seed);
        ...
        expedition.MissionParams = _planetParams;

        var landingPadRadius = 24;
        // var minDungeonOffset = landingPadRadius + 4;
        //
        // KS14 certified comment start
        // Im ripping dungeons from planets because im expecting to use premapped solutions instead
        // I really dont want to adjust dungeons code for future stuff ill be adding to planets
        // And frankly i just dont like current dungeon gen.
        // KS14 certified comment end
        //
        // We'll use the dungeon rotation as the spawn angle
        // var dungeonRotation = _dungeon.GetDungeonRotation(_planetParams.Seed);
